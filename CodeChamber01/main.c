...
struct CompareArray Array[512] = { 0 }; // name 저장용 구조체 배열
...
/*
int ChangeCheck(char* Compare) // 비교검사 함수
{
    long TxtSearch;
    int NextSearch = 1;

    int CheckResult = 1; // CheckResult 초기화

    TxtSearch = _findfirst("D:\\target\\*.txt", &FileData); // 첫 txt 파일 검사

    while (NextSearch != -1)
    {
        int cmp = strcmp(FileData.name, Compare); // 획득한 name 멤버와 이름 비교

        if (cmp == 0)
        {
            CheckResult = 0; // 동일 name이 발견되면 0으로 CheckResult 스위칭
        }

        NextSearch = _findnext(TxtSearch, &FileData); // 다음 txt 파일 검사 진행
    }
    _findclose(TxtSearch);

    return CheckResult; // 중복 검사 결과 반환
}
*/